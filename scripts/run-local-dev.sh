#!/bin/bash
# Bash script for running backend and frontend in development mode with live reload
# Handles proper process management and graceful shutdown on Ctrl+C

set -e

echo "Starting Traefik Dynamic Editor - Local Development with Live Reload"
...
echo "[Frontend] Starting dev server with live reload..."
cd "$FRONTEND_DIR"
npm run dev &
FRONTEND_PID=$!
PIDS+=($FRONTEND_PID)
